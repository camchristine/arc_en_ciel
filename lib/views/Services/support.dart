...
class _SupportState extends State<Support> {
  ...
  Future<void> openWhatsappSuppport() async {
    ...
    Uri whatsappURlAndroid = Uri.parse(
        "https://wa.me/$whatsappTel?text=${Uri.parse("Bonjour, Pouvez-vous m'aider svp ? mon nom c'est ${_cUser.user.nom} ${_cUser.user.prenom}. L'école est : Saint Viateur")}");
  ...
  Future<void> _launchRating() async {
    final Uri url = Uri.parse(
        "https://play.google.com/store/apps/details?id=com.lkmdigital.saint viateur&pli=1");
  ...
  Future<void> _launchFeedback() async {
    final Uri url = Uri.parse(
        "mailto:infos@lkmdigital.ci?subject=FeedBack EPCND2P [${_cUser.user.telephone}]&");
    if (!await launchUrl(url)) {
      throw Exception('Could not launch $url');
    }
  }
